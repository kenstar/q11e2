...
//
// $Id: q11e2_kcaptcha.inc.php,v 2.30 2006/04/01 10:40:00 ks Exp $
//
...
function plugin_q11e2_kcaptcha_action()
{
	if(isset($_SESSION['captcha_keystring']) && $_SESSION['captcha_keystring'] ==  $_POST['keystring']){
	} else if(! isset($_SESSION['captcha_keystring'])){
		return array('msg'=>'unset session_start() ');
	} else {
...
//header化 by io
  ...
  foreach ($postdata_old as $line)
  {
    ...
    }else if (preg_match('/^#q11e2_kcaptcha\(/',$line) and $comment_no++ == $post['comment_no']){
      // #q11e2_kcaptcha記述の次の行に、投稿されたメッセージを記述する。そのための$switch=1
      $switch = 1;
      $postdata .= $line;
    }else if ( $switch == 2 ){
	// header直後の回答
  ...
  if ($switch == 1){
    // #q11e2_kcaptchaが最後の行に記述されている場合は、上のループから抜けてしまう。
    // そのための対応
    $postdata .= "$header\n$comment\n";
  }	   	    
  $switch = 0;
...
function plugin_q11e2_kcaptcha_convert()
{
...
<!-- header化 by io <tr><td class="style_td">項目</td><td class="style_td">選択肢 -->
 ...
 <thead><td class="style_td">項目</td><td class="style_td">選択肢
  ...
  <input type="hidden" name="plugin" value="q11e2_kcaptcha" />
...
<tr><td class="style_td"> </td><td class="style_td">
  ...
  <input type="text" name="keystring" size="$cpt_input_size" maxlength="$cpt_input_max" />
 ($desc_str) 
  <input type="submit" name="q11e2_kcaptcha" value="回答" />
</td></tr>
 </form>
</tbody></table></div><br />
EOD;
